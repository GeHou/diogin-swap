...
<div id="page">
    ...
    <ul class="messages">
        ...
        <?php else: ?>
            ...
            <?php foreach ($messages as $message): ?>
                <li class="message">
                    <p class="guest"><?php if ($message->site === ''): ?><?php echo $message->author; ?><?php else: ?><a href="<?php echo $message->site; ?>"><?php echo $message->author; ?></a><?php endif; ?><?php if ($logined): ?>(<?php echo $message->email; ?>)<?php endif; ?> 留言于 <?php echo date('Y-m-d H:i:s', $message->pub_time); ?></p>
                    <div class="content"><?php show_ubb($message->content); ?></div>
                    <?php if ($logined): ?>
                        <ul class="admin">
                            <li><a href="<?php self::php_url('message/edit?id=' . $message->id); ?>">编辑</a></li>
                            <li><a href="<?php self::csrf_url('member', 'message/delete?id=' . $message->id); ?>" onclick="return delete_message(this);">删除</a></li>
                        </ul>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
    ...
    <form id="new_form" action="<?php self::php_url('message/new'); ?>" method="POST" enctype="application/x-www-form-urlencoded">
        <h2>添加留言</h2>
