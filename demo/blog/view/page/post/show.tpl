...
<div id="page">
    ...
    <div class="post">
        <h2 class="title"><a href="<?php self::php_url('post/show?id=' . $post->id); ?>"><?php echo $post->title; ?></a></h2>
        <p class="author"><?php echo $post->member->name; ?> 发表于 <?php echo date('Y-m-d H:i:s', $post->pub_time); ?></p>
        <div class="article"><?php show_ubb($post->content); ?></div>
        <ul class="meta">
            <li>类型：<a href="<?php self::php_url('post/index?category_id=' . $post->category->id); ?>"><?php echo $post->category->name; ?></a></li>
            <li>阅读：<?php echo $post->read_count; ?></li>
            <li>评论：<?php echo $post->comment_count; ?></li>
            <li>标签：
            <?php foreach ($post->post_tag as $tag): /* 遍历所有标签 */ ?>
                <a href="<?php self::php_url(array('post/index', array('tag' => $tag->name))); ?>"><?php echo $tag->name; ?></a>
            <?php endforeach; ?></li>
        </ul>
        <?php if ($logined): /* 如果是管理员 */ ?>
            <ul class="admin">
                <li><a href="<?php self::php_url('post/edit?id=' . $post->id); ?>">编辑</a></li>
                <li><a href="<?php self::csrf_url('member', 'post/delete?id=' . $post->id); ?>" onclick="return delete_post(this);">删除</a></li>
            </ul>
        <?php endif; ?>
    </div>
    
    <ul class="comments">
        <?php if ($comments === null): /* 如果没有评论 */ ?>
        ...
        <?php else: /* 如果有评论 */ ?>
            ...
            <?php foreach ($comments as $comment): /* 遍历所有评论 */ ?>
                ...
                <li class="comment" id="<?php echo $comment->id; ?>">
                    <p class="commenter"><?php if ($comment->site === ''): ?><?php echo $comment->author; ?><?php else: ?><a href="<?php echo $comment->site; ?>"><?php echo $comment->author; ?></a><?php endif; ?><?php if ($logined): ?>(<?php echo $comment->email; ?>)<?php endif; ?> 评论于 <?php echo date('Y-m-d H:i:s', $comment->pub_time); ?></p>
                    <div class="content"><?php show_ubb($comment->content); ?></div>
                    <?php if ($logined): /* 如果是管理员 */ ?>
                        <ul class="admin">
                            <li><a href="<?php self::php_url('comment/edit?id=' . $comment->id); ?>">编辑</a></li>
                            <li><a href="<?php self::csrf_url('member', 'comment/delete?id=' . $comment->id); ?>" onclick="return delete_comment(this);">删除</a></li>
                        </ul>
                    <?php endif; ?>
                </li>
    ...
    <form id="new_form" action="<?php self::php_url('comment/new?post_id=' . $post->id); ?>" method="POST" enctype="application/x-www-form-urlencoded">
        <h2>添加评论</h2>
