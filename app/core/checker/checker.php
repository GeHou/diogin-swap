...
abstract class checker {
    ...
    protected function check_cannot_in($prop_name, $prop_value, $rule_value) {
        ...
        if (is_array($prop_value)) {
            if (array_intersect($prop_value, $value_list) !== array()) {
                return $reason;
            }
    ...
    protected function check_should_in($prop_name, $prop_value, $rule_value) {
        list($value_list, $reason) = $rule_value;
        if (is_array($prop_value)) {
            if (array_diff($prop_value, $value_list) !== array()) {
                return $reason;
            }
        } else if (!in_array($prop_value, $value_list)) {
            return $reason;
    ...
    protected function check_value_type($prop_name, $prop_value, $rule_value) {
        list($type_name, $reason) = $rule_value;
        $value_type = 'swap\\' . $type_name . '_value';
        $check_function = 'is_valid_' . $type_name;
        if (!$value_type::$check_function($prop_value)) {
            return $reason;
        }
    }
    ...
    protected $checked_props = array();
    protected $prop_rules = array();
