...
abstract class sqlite_rdb_node extends rdb_node {
    ...
    public function get_for_fields($field_names, $table_name, array $keyvalues, array $order_limit = array(array(), 0, 0)) {
        return $this->do_get(sqlite_rdb_node_util::build_field_name_list($field_names), $table_name, $keyvalues, $order_limit);
    }
    ...
    public function get_where_for_fields($field_names, $table_name, $where, array $args = array(), array $order_limit = array(array(), 0, 0)) {
        return $this->do_get_where(sqlite_rdb_node_util::build_field_name_list($field_names), $table_name, $where, $args, $order_limit);
    }
    public function get_by_id_for_fields($field_names, $table_name, $id) {
        return $this->do_get_by_id(sqlite_rdb_node_util::build_field_name_list($field_names), $table_name, $id);
    }
    ...
    public function get_in_for_fields($field_names, $table_name, $field_name, array $values, array $order_limit = array(array(), 0, 0)) {
        return $this->do_get_in(sqlite_rdb_node_util::build_field_name_list($field_names), $table_name, $field_name, $values, $order_limit);
    }
    public function get_all_for_fields($field_names, $table_name, array $order_limit = array(array(), 0, 0)) {
        return $this->do_get_all(sqlite_rdb_node_util::build_field_name_list($field_names), $table_name, $order_limit);
    }
    
    public function count($table_name, array $keyvalues) {
        $count_name = sqlite_rdb_node_util::build_field_name('count');
        $table_name = $this->get_full_table_name($table_name);
        $conditions = sqlite_rdb_node_util::build_equal_list($keyvalues, ' AND ');
    ...
    public function count_where($table_name, $where, array $args = array()) {
        $count_name = sqlite_rdb_node_util::build_field_name('count');
        $table_name = $this->get_full_table_name($table_name);
        $conditions = sqlite_rdb_node_util::replace_sql_args(sqlite_rdb_node_util::localize_where($where), $args);
        return $this->do_count("SELECT COUNT(*) AS {$count_name} FROM {$table_name} WHERE {$conditions}");
    }
    public function count_by_ids($table_name, array $ids) {
        ...
        $count_name = sqlite_rdb_node_util::build_field_name('count');
        $table_name = $this->get_full_table_name($table_name);
        $id_name = sqlite_rdb_node_util::build_field_name('id');
        $id_list = sqlite_rdb_node_util::build_value_list($ids);
        // @todo: optimize using covering index
    ...
    public function count_in($table_name, $field_name, array $values) {
        ...
        $count_name = sqlite_rdb_node_util::build_field_name('count');
        $table_name = $this->get_full_table_name($table_name);
        $field_name = sqlite_rdb_node_util::build_field_name($field_name);
        $value_list = sqlite_rdb_node_util::build_value_list($values);
        // @todo: optimize using covering index
    ...
    public function select($sql, array $args = array()) {
        return $this->query_and_fetch_records(sqlite_rdb_node_util::replace_sql_args($sql, $args), false);
    }
    ...
    protected function do_get($field_name_list, $table_name, array $keyvalues, array $order_limit = array(array(), 0, 0)) {
        $table_name = $this->get_full_table_name($table_name);
        $conditions = sqlite_rdb_node_util::build_equal_list($keyvalues, ' AND ');
        return $this->query_and_fetch_records("SELECT {$field_name_list} FROM {$table_name} WHERE {$conditions}" . sqlite_rdb_node_util::build_order_limit_sql($order_limit), true);
    }
    ...
    protected function do_get_in($field_name_list, $table_name, $field_name, array $values, array $order_limit = array(array(), 0, 0)) {
        ...
        $field_name = sqlite_rdb_node_util::build_field_name($field_name);
        $value_list = sqlite_rdb_node_util::build_value_list($values);
        return $this->query_and_fetch_records("SELECT {$field_name_list} FROM {$table_name} WHERE {$field_name} IN ({$value_list})" . sqlite_rdb_node_util::build_order_limit_sql($order_limit), true);
    }
    protected function do_get_all($field_name_list, $table_name, array $order_limit = array(array(), 0, 0)) {
        $table_name = $this->get_full_table_name($table_name);
        return $this->query_and_fetch_records("SELECT {$field_name_list} FROM {$table_name}" . sqlite_rdb_node_util::build_order_limit_sql($order_limit), true);
    }
    ...
    protected function do_count($sql) {
        ...
        if ($result === false) {
            throw new local_except("select error: {$sql}");
        }
    ...
    protected function do_pager($record_count, $table_name, array $keyvalues, array $order_limit = array(array(), 0, 0)) {
        rdb_node_util::restrict_order_limit($order_limit, $record_count);
    ...
    protected function query_and_fetch_records($sql, $index_by_id) {
        ...
        if ($index_by_id) {
            while (($record = $result->fetch_record()) !== null) {
                // @todo: 如果 $record['id'] 已经是 int，则去除强制类型转换
    ...
    protected function get_full_table_name($table_name) {
        return sqlite_rdb_node_util::build_table_name($table_name);
    }
}
// [类型] sqlite 关系数据库主节点
class sqlite_master_rdb_node extends sqlite_rdb_node {
    public function set($table_name, array $keyvalues, array $conditions) {
        $table_name = $this->get_full_table_name($table_name);
        $modifies = sqlite_rdb_node_util::build_equal_list($keyvalues, ', ');
        $conditions = sqlite_rdb_node_util::build_equal_list($conditions, ' AND ');
    ...
    public function update($sql, array $args = array()) {
        $this->execute(sqlite_rdb_node_util::replace_sql_args($sql, $args));
        return $this->affected_rows();
    }
    ...
    public function inc($table_name, array $keyvalues, array $conditions) {
        $table_name = $this->get_full_table_name($table_name);
        $modifies = sqlite_rdb_node_util::build_inc_dec_list($keyvalues, '+');
        $conditions = sqlite_rdb_node_util::build_equal_list($conditions, ' AND ');
        $sql = "UPDATE {$table_name} SET {$modifies} WHERE {$conditions}";
        $this->execute($sql);
        return $this->affected_rows();
    ...
    public function set_and_inc($table_name, array $sets, array $incs, array $conditions) {
        $table_name = $this->get_full_table_name($table_name);
        $modifies = sqlite_rdb_node_util::build_equal_list($sets, ', ') . ', ' . sqlite_rdb_node_util::build_inc_dec_list($incs, '+');
        $conditions = sqlite_rdb_node_util::build_equal_list($conditions, ' AND ');
        $sql = "UPDATE {$table_name} SET {$modifies} WHERE {$conditions}";
        $this->execute($sql);
        return $this->affected_rows();
    ...
    protected function do_add_or_rep($method, $table_name, array $keyvalues) {
        ...
        foreach ($keyvalues as $key => $value) {
            $columns[] = sqlite_rdb_node_util::build_field_name($key);
            // @todo: SQLite3 may be not exist
            $values[]  = is_int($value) ? $value : ("'" . SQLite3::escapeString((string)$value) . "'");
        }
    ...
    protected function execute($sql) {
        ...
        if (!$result) {
            throw new local_except("execute error: {$sql}");
        }
    }
...
// [实体] sqlite 关系数据库节点工具
class sqlite_rdb_node_util extends rdb_node_util {
    public static function build_order_limit_sql(array $order_limit) {
        ...
        if ($page_size !== 0) {
            ...
            $limit_sql .= " LIMIT {$page_size} OFFSET {$begin_offset}";
        }
    ...
    public static function build_value_list(array $values) {
        foreach ($values as $key => $value) {
            if (!is_int($value)) {
                // @todo: SQLite3 may be not exist
                $values[$key] = "'" . SQLite3::escapeString((string)$value) . "'";
            }
        }
    ...
    public static function build_equal_list(array $keyvalues, $separator) {
        ...
        foreach ($keyvalues as $key => $value) {
            ...
            if (is_int($value)) {
                ...
                // @todo: SQLite3 may be not exist
                $equal .= "'" . SQLite3::escapeString((string)$value) . "'";
            }
            $equal_list[] = $equal;
        }
    ...
    public static function replace_sql_args($sql, array $args) {
        ...
        foreach ($args as $arg) {
            if (is_null($arg)) {
                ...
                // @todo: SQLite3 may be not exist
                $replace_str = '\'' . SQLite3::escapeString($arg) . '\'';
            } else {
                $replace_str = $arg;
            }
    ...
    public static function build_table_name($table_name) {
        // SQLite supports many quote method. Choose mysql way
        return '`' . $table_name . '`';
    }
